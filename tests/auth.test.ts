...
describe("Authentication System", () => {
	...
	describe("Error Handling - Unknown Errors", () => {
		it("should handle unknown errors in register gracefully", async () => {
		...
		it("should handle unknown errors in login gracefully", async () => {
			// Register first
			await request(app).post("/api/auth/register").send(testUser);
		...
		it("should handle unknown errors in profile retrieval gracefully", async () => {
			const res = await request(app)
				.post("/api/auth/register")
				.send(testUser);
			...
			expect(profileRes.status).toBeLessThan(600);
			expect(profileRes.body).toHaveProperty("success");
		});
	});

	describe("Validation Error Paths", () => {
		it("should return validation errors for register with invalid data", async () => {
		...
		it("should return validation errors for login with missing fields", async () => {
			const res = await request(app).post("/api/auth/login").send({
	...
	describe("Edge Cases - User Not Found", () => {
		it("should return 404 when user is not found in getProfile", async () => {
